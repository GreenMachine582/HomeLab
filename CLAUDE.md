...
- **Infisical and Semaphore have two access paths**: (1) Caddy in `homelab-edge-services` provides HTTPS routes on ports 8443/8444 (UFW: `lan_subnet` + `tailscale_cgnat_range`) — use `infisical.homelab.local:8443` / `semaphore.homelab.local:8444` from LAN, or `homelab-edge:8443` / `homelab-edge:8444` from Tailscale; (2) direct ports 8222/3010 remain scoped to `tailscale_cgnat_range` for non-browser clients. Do not widen ports 8222/3010 beyond the Tailscale CGNAT range.
