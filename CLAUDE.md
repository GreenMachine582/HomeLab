...
- **Infisical and Semaphore have two access paths**: (1) Caddy in `homelab-edge-services` provides HTTPS on ports 8443/8444 scoped to `tailscale_cgnat_range` — use `homelab-edge.<tailnet>.ts.net:8443` / `homelab-edge.<tailnet>.ts.net:8444` from Tailscale (browser-trusted cert via `tailscale cert`); (2) direct ports 8222/3010 remain scoped to `tailscale_cgnat_range` for non-browser clients. Do not widen ports 8222/3010 or 8443/8444 beyond the Tailscale CGNAT range.
