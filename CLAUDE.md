...
- **Infisical and Semaphore have two access paths**: (1) Caddy in `homelab-edge-services` provides HTTPS LAN routes on `infisical.homelab.local:8443` and `semaphore.homelab.local:8444` (UFW: `lan_subnet`); (2) direct ports 8222/3010 remain scoped to `tailscale_cgnat_range` for non-browser clients and Tailscale access. Do not widen ports 8222/3010 beyond the Tailscale CGNAT range.
