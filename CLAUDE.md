...
- **Infisical and Semaphore have two access paths**: (1) Caddy in `homelab-edge-services` provides HTTPS routes on ports 8443/8444 (UFW: `lan_subnet` + `tailscale_cgnat_range`) — use `infisical.homelab.local:8443` / `semaphore.homelab.local:8444` from LAN (`tls internal`, browser warns without CA install), or `homelab-edge.<tailnet>.ts.net:8443` / `homelab-edge.<tailnet>.ts.net:8444` from Tailscale (browser-trusted cert via `tailscale cert`); (2) direct ports 8222/3010 remain scoped to `tailscale_cgnat_range` for non-browser clients. Do not widen ports 8222/3010 beyond the Tailscale CGNAT range.
