# Homelab Topology & Shared Data Tier — Planning Brief (v7)

> Companion to [repo_split_brief.md](./repo_split_brief.md). That brief answers *how services are packaged and deployed*; this one answers *where they run* and *where their state lives*. It builds on the decided items there (central `services.yml`, `deploy-service` on edge, bootstrap-tier circularity, cloudflared rolling constraint) and changes none of them. For current roadmap/status and hardware-expansion planning, see [consolidated_brief.md](./consolidated_brief.md).

**Status:** Current roadmap and step status is now tracked in [`consolidated_brief.md`](./consolidated_brief.md) (§8/§9) — see there for up-to-date state, including the data-tier device decision (§7 below, resolved: rpi-03) and the new `data`/`media` host_roles. This doc's own design content — devices-as-data model (§3), co-residency audit (§5), MagicDNS/`ADDR_<REPO>` addressing (§3.5) — is unchanged; only status/migration-step tracking moved.
**Origin repo:** https://github.com/GreenMachine582/HomeLab

**Changelog from v6:**
- **Roadmap/status tracking moved to [`consolidated_brief.md`](./consolidated_brief.md).** §6's Migration Plan step numbering (Steps 1–7) is superseded by that brief's Stage 0–8 mapping (its §9 reconciliation table maps every step to a Stage). §7 Open items are largely resolved there too: the data-tier device is confirmed as rpi-03 (re-roled `homelab-data-01`); Redis isolation is still open, carried into the new brief's §10. §§1–5 (design rationale) are untouched.
